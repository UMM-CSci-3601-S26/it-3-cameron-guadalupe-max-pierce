...
//     cy.wait(100);
//     cy.get('mat-option').contains('Morris Area Elementary School (MAES)').click();
...
//     // Add second child
//     cy.contains('Add Another Student').click();
//     cy.get('input[name="childFirstName1"]').type('Emma');
//     cy.get('input[name="childLastName1"]').type('Smith');
//     cy.get('mat-select').click(); //Screw this. Fricking cypress, I swear to god
//     cy.wait(100);
//     cy.get('mat-option').contains('Morris Area Elementary School (MAES)').click();
...
//     cy.get('.child-form button').contains('Remove Student').last().click();
